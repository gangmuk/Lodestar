...
#     # hyperparameters = read_hyperparameters_from_file('hyperparameters.txt')
...
#     # NOW run root cause analysis ONCE after training
#     logger.info("🔍 Running comprehensive root cause analysis...")
#     try:
#         # Create evaluation data for analysis
#         eval_indices = torch.randperm(len(dataset))[:min(500, len(dataset))]
#         eval_data_for_analysis = {
#             'pod_features_with_staleness': combined_data['pod_features_with_staleness'][eval_indices],
#             'kv_hit_ratios': combined_data['kv_hit_ratios'][eval_indices],
#             'request_features': combined_data['request_features'][eval_indices],
...
#         analysis_results = comprehensive_root_cause_analysis(agent, combined_data, eval_data_for_analysis)
...
def analyze_pod_identity_learning(agent, eval_data, output_dir):
    ...
    with torch.no_grad():
        ...
        if len(set(hist_rewards)) > 1:  # Only if there's variance
            ...
            logger.info(f"\nCorrelation between historical rewards and preferences: {correlation:.3f}")
            
            if correlation > 0.7:
                logger.info("  ✅ STRONG correlation - model learned from historical performance")
            elif correlation > 0.3:
                logger.info("  📊 MODERATE correlation - some learning detected")
            else:
                logger.info("  ⚠️  WEAK correlation - model may not be learning effectively")
