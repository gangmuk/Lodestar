...
def handle_infer():
    ...
    try:
        ...
        # features_must_exist_in_stats_instance = [feature for feature in processed_df.columns if "last" not in feature and feature not in non_interest]
        ...
        for feature in features_must_exist_in_stats_instance:
            if feature not in stats_instance.feature_stats:
                ...
                assert False
        logger.info(f"features_must_exist_in_stats_instance: {features_must_exist_in_stats_instance}")
        for feature in processed_df.columns:
            if feature in stats_instance.feature_stats:
                stats = stats_instance.feature_stats[feature]
                mean_val = stats.get_mean().item()
                std_val = stats.get_std().item()
...
def init():
    ...
    for feature_name, stats in stats_instance.feature_stats.items():
        logger.info(f"stats_instance, {feature_name}: count={stats.count}, mean={stats.mean}, std={stats.get_std()}")
