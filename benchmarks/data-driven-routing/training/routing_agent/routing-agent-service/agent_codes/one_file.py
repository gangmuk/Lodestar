    ...
    if is_training:
        ...
        if feature_std == 0 or np.isclose(feature_std, 0, atol=1e-10):
            ...
            try:
                ...
                logger.error(f"Error adding constant feature {feature} to CONFIG: {e}")
                logger.error(f"feature: {feature}")
                logger.error(f"CONFIG content: {stats_instance.CONFIG}")
                logger.error(f"type(feature): {type(feature)}")
                logger.error(f"type(stats_instance.CONFIG): {type(stats_instance.CONFIG)}")
                assert False
            try:
                stats_instance.CONFIG["NUM_CONSTANT_FEATURES"] = len(stats_instance.CONFIG.get("CONSTANT_FEATURES", set()))
            except Exception as e:
                logger.error(f"Error updating NUM_CONSTANT_FEATURES in CONFIG: {e}")
                logger.error(f"feature: {feature}")
                logger.error(f"CONFIG content: {stats_instance.CONFIG}")
                logger.error(f"type(feature): {type(feature)}")
                logger.error(f"type(stats_instance.CONFIG): {type(stats_instance.CONFIG)}")
