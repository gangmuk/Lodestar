...
experiment_configs=(
    "contextual_bandit_perpodmodel_checkpoint_ttft_negative_linear_random-onlinelearning_1|mooncake|toolagent-2-extended-ver1|${target_gpu}|6,8,16,22|1"
    "prefix_cache_1|mooncake|toolagent-2-extended-ver1|${target_gpu}|6,8,16,22|1"
)
...
# Autoscaling plan: start with 5 A30 + 4 V100 pods (9 total), scale up to
# 8 A30 + 8 V100 pods (16 total) mid-experiment when RPS increases.
v100_llm_model="llama-3-8b-instruct-v100"
v100_initial_replicas=4   # start with 4 V100 pods
v100_scaleup_replicas=8   # scale up to 8 V100 pods when RPS increases
a30_initial_replicas=5    # start with 5 A30 pods
a30_scaleup_replicas=8    # scale up to 8 A30 pods when RPS increases
...
AIBRIX_PREFIX_CACHE_BLOCK_NUMBER=250000 # fixed across scale-up — mid-run change causes gateway pod restart
...
for i in $(seq 0 $((num_experiments-1))); do
    IFS='|' read -r routing workload workload_nm gpu rps episodes _ <<< "${experiment_configs[$i]}"
    echo "  $((i+1)). routing=${routing}, workload=${workload}/${workload_nm}, gpu=${gpu}, rps=${rps}, episodes=${episodes}"
done
echo "========================================="
    ...
    for MAX_NUM_ONLINE_TRAINS in "${MAX_NUM_ONLINE_TRAINS_LIST[@]}"; do
        for CANDIDATE_K in "${CANDIDATE_K_LIST[@]}"; do
            ...
            for experiment_idx in $(seq 0 $((num_experiments-1))); do
                ...
                retry_command bash -c "kubectl get deploy ${v100_llm_model} -o yaml > ${experiment_result_output_dir}/${v100_llm_model}.yaml"
                ...
                # Scale-up function: time-based trigger to scale both A30 and V100 deployments
                # 1 minute BEFORE the RPS transition (halfway point of requests).
                # Watches client log for "Scheduling N tasks" to get total requests,
                # then sleeps until lead_time_sec before the estimated transition.
                # (Old approach parsed completed request IDs, but requests complete
                #  out of order, causing the trigger to fire minutes late.)
                scaleup_at_rps_transition() {
                    local client_log_file=$1
                    local a30_deploy=$2
                    local a30_target=$3
                    local v100_deploy=$4
                    local v100_target=$5
                    local first_segment_rps=$6
                    local lead_time_sec=60  # scale up 1 minute before RPS change

                    echo "[scaleup] Watching client log for RPS transition..."
                    echo "[scaleup] Will scale ${a30_deploy} to ${a30_target} and ${v100_deploy} to ${v100_target} replicas, ${lead_time_sec}s before RPS change"

                    # Wait for the client log file to exist
                    while [ ! -f "${client_log_file}" ]; do
                        sleep 1
                    done

                    # Wait for the scheduling line to determine total requests
                    local total_requests=""
                    while [ -z "$total_requests" ]; do
                        total_requests=$(grep -oP 'Scheduling \K\d+(?= tasks)' "${client_log_file}" | head -1)
                        sleep 1
                    done

                    # Record when requests started being sent
                    local start_epoch=$(date +%s)

                    # Compute first segment duration: (total/2) requests at first_segment_rps
                    local first_segment_requests=$((total_requests / 2))
                    local first_segment_duration=$((first_segment_requests / first_segment_rps))
                    local sleep_duration=$((first_segment_duration - lead_time_sec))
                    if [ "$sleep_duration" -lt 0 ]; then
                        sleep_duration=0
                    fi

                    echo "[scaleup] total_requests=${total_requests}, first_segment=${first_segment_requests} reqs at ${first_segment_rps} rps = ~${first_segment_duration}s"
                    echo "[scaleup] Sleeping ${sleep_duration}s (${first_segment_duration}s - ${lead_time_sec}s lead time), then scaling up"

                    sleep ${sleep_duration}

                    local trigger_epoch=$(date +%s)
                    local elapsed=$((trigger_epoch - start_epoch))
                    echo "[scaleup][$(date)] Timer fired after ${elapsed}s. Scaling ${lead_time_sec}s before estimated RPS transition!"

                    echo "[scaleup][$(date)] Scaling ${a30_deploy} replicas: ${a30_initial_replicas} -> ${a30_target}"
                    kubectl scale deployment ${a30_deploy} --replicas=${a30_target} --namespace default

                    echo "[scaleup][$(date)] Scaling ${v100_deploy} replicas: ${v100_initial_replicas} -> ${v100_target}"
                    kubectl scale deployment ${v100_deploy} --replicas=${v100_target} --namespace default

                    echo "[scaleup][$(date)] Scale commands issued. Pods will come up gradually."
                }

                # Scale deployments to initial replicas before experiment starts
                echo "Scaling ${llm_model} to ${a30_initial_replicas} A30 replicas before experiment..."
                kubectl scale deployment ${llm_model} --replicas=${a30_initial_replicas} --namespace default
                kubectl rollout status deployment/${llm_model} --namespace default --timeout=120s || true
                echo "${llm_model} scaled to ${a30_initial_replicas} replicas"

                echo "Scaling ${v100_llm_model} to ${v100_initial_replicas} V100 replicas before experiment..."
                kubectl scale deployment ${v100_llm_model} --replicas=${v100_initial_replicas} --namespace default
                kubectl rollout status deployment/${v100_llm_model} --namespace default --timeout=120s || true
                echo "${v100_llm_model} scaled to ${v100_initial_replicas} replicas"
                ...
                # Start scale-up watcher in background (triggers 1min before RPS transitions from 10 to 22)
                # Parse first-segment RPS from comma-separated rps string (e.g., "10,22" -> 10)
                first_segment_rps=$(echo "${rps}" | cut -d',' -f1)
                scaleup_at_rps_transition "${experiment_result_output_dir}/client.log.txt" "${llm_model}" "${a30_scaleup_replicas}" "${v100_llm_model}" "${v100_scaleup_replicas}" "${first_segment_rps}" &
                pid_scaleup=$!
                echo "Started scale-up watcher (pid=${pid_scaleup}, first_segment_rps=${first_segment_rps}, A30: ${a30_initial_replicas}->${a30_scaleup_replicas}, V100: ${v100_initial_replicas}->${v100_scaleup_replicas})"
                ...
                # Scale both deployments back to initial replicas after experiment
                echo "Scaling ${llm_model} back to ${a30_initial_replicas} A30 replicas..."
                kubectl scale deployment ${llm_model} --replicas=${a30_initial_replicas} --namespace default
                echo "Scaling ${v100_llm_model} back to ${v100_initial_replicas} V100 replicas..."
                kubectl scale deployment ${v100_llm_model} --replicas=${v100_initial_replicas} --namespace default
                ...
                kill $pid_scaleup 2>/dev/null || true
