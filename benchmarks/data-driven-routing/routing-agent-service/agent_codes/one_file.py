...
class RunningStats:
    ...
    def update_stats_incrementally(self, new_data):
        ...
        if self.count == 0: # The very first update
            ...
            logger.info(f"qwer, {self.feature_names}, new_count={new_count}, total_count={self.count} samples, old_mean={old_mean}, old_std={old_std}, old_sum_sq_diff={old_sum_sq_diff}, old_min={old_min}, old_max={old_max}, new_mean={self.mean}, new_std={self.std}, new_sum_sq_diff={self.sum_sq_diff}, new_min={self.min}, new_max={self.max}")
            logger.info(f"qwer, {self.feature_names}, new_count={new_count}, total_count={self.count} samples, old_mean={old_mean}, old_std={old_std}, old_sum_sq_diff={old_sum_sq_diff}, old_min={old_min}, old_max={old_max}, new_mean={self.mean}, new_std={self.std}, new_sum_sq_diff={self.sum_sq_diff}, new_min={self.min}, new_max={self.max}")
            return
        ...
        if self.min is None or self.max is None:
            ...
            self.min = np.minimum(self.min, np.min(new_data, axis=0)) 
            self.max = np.maximum(self.max, np.max(new_data, axis=0))
        logger.info(f"asdf, {self.feature_names}, new_count={new_count}, total_count={self.count} samples, old_mean={old_mean}, old_std={old_std}, old_sum_sq_diff={old_sum_sq_diff}, old_min={old_min}, old_max={old_max}, new_mean={self.mean}, new_std={self.std}, new_sum_sq_diff={self.sum_sq_diff},  new_min={self.min}, new_max={self.max}")
...
def _get_normalizable_features(processed_df):
    ...
    for col in processed_df.columns:
        if col.startswith('pod_') and processed_df[col].dtype in ['float64', 'int64'] and 'gpu_model' not in col:
...
def normalize_features_for_training(processed_df, stats_instance: PerFeatureRunningStats) -> pd.DataFrame:
    ...
    logger.info(f"🔍 Normalizing features: {normalizable_features}")
...
#     normalizable_features, non_normalizable_features = _get_normalizable_features(df_copy)
...
def preprocess_single_row_fast(parsed_df, RL_MODEL_HYPERPARAMETERS):
    ...
    pod_metrics = row.get('podMetricsLastSecond', {})
    # ## I was weirdly using podMetricsLastSecond to get pod ids...
