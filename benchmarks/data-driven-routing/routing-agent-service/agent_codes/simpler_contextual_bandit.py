...
class FixedPolicyNetwork(nn.Module):
    ...
    def get_action(self, pod_features, kv_hit_ratios, request_features, explore, epsilon):
        ...
        if not explore:
            ...
            explore_mask = torch.zeros(actions.shape[0], dtype=torch.long, device=actions.device)  # All exploitation
            return actions, log_probs, explore_mask
        ...
        explore_mask = (torch.rand(batch_size, device=device) < epsilon).long() # 0 or 1. 0: exploitation, 1: exploration
        actions = (1 - explore_mask) * greedy_actions + explore_mask * random_actions # choose greedy(inference) or random
        log_probs = torch.log(torch.gather(action_probs, 1, actions.unsqueeze(1)).squeeze(1) + 1e-10)
...
def infer_from_tensor(tensor_data, request_id, model_updated, HYPERPARAMETERS, final_model_dir):
    ...
    with torch.no_grad():
        ...
        if HYPERPARAMETERS['exploration_rate'] > 0:
            action, _, explore_mask = agent.policy.get_action(pod_features, kv_hit_ratios, request_features, explore=HYPERPARAMETERS['explore'], epsilon=HYPERPARAMETERS['exploration_rate'])
