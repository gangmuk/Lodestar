...
def _draw_vertical_arrow(ax, x_frac=0.95, y_top=0.50, y_bot=0.05,
                         label='More cache benefit', width=0.07):
...
def panel_a(ax, sel_kv, ttft):
    ...
    ax.scatter(sel_kv[idx], ttft[idx], s=5, alpha=0.12, color=C_BLUE,
    ...
    ax.set_title('(a)  Nonlinear relationship:\nPearson $\\approx$ 0 yet Spearman = $-$0.32')
...
def panel_b_load(ax, sel_waiting, ttft):
    ...
    ax.scatter(sel_waiting[idx] + jitter_x, ttft[idx], s=5, alpha=0.12, color=C_ORANGE,
               rasterized=True, edgecolors='none', label='Samples')
    ...
    ax.annotate(
        'Pearson  r = {:+.3f}\nSpearman $\\rho$ = {:+.3f}'.format(r_p, r_s),
        xy=(0.97, 0.97), xycoords='axes fraction', fontsize=8,
        ha='right', va='top',
        bbox=dict(boxstyle='round,pad=0.3', fc='white', ec='gray',
                  alpha=0.9, linewidth=0.5))
...
def panel_b(ax, sel_kv, input_tokens):
    ...
    ax.annotate(f'Spearman $\\rho$ = {r_s:+.3f}', xy=(0.97, 0.97),
    ...
    ax.set_title('(c)  Confound: shorter requests\nget higher cache hits')
...
def panel_c(ax, sel_kv, ttft, input_tokens):
    ...
    for x, y, n in zip(centers, medians, counts):
        ax.annotate(f'n={n}', xy=(x, y), fontsize=5.5, ha='center',
    ...
    ax2.set_ylabel('Mean input tokens', color=C_ORANGE, fontsize=8)
...
# ---------------------------------------------------------------------------
def panel_d(ax, sel_kv, ttft, input_tokens):
    quantiles = np.percentile(input_tokens, [0, 25, 50, 75, 100])
    ...
    for i in range(4):
        ...
        mask = ((input_tokens >= lo_q) & (input_tokens <= hi_q) if i == 3
                else (input_tokens >= lo_q) & (input_tokens < hi_q))
        bkv = sel_kv[mask]
        bttft = ttft[mask]
        med = np.median(bkv)
    ...
    x = np.arange(4)
    w = 0.32
    bars_lo = ax.bar(x - w / 2, lo_vals, w, label='Low KV hit', color=C_RED,
                     alpha=0.85, edgecolor='white', linewidth=0.5)
    bars_hi = ax.bar(x + w / 2, hi_vals, w, label='High KV hit', color=C_BLUE,
                     alpha=0.85, edgecolor='white', linewidth=0.5)
    ...
    ax.set_title('(e)  Controlled analysis: sign reversal\nacross request length quartiles')
...
def panel_e(ax, sel_kv, ttft, sel_waiting):
    ...
    for j in range(len(wait_bins) - 1):
        ...
        if mask.sum() > 50:
            ...
            rho_vals.append(rho)
            bin_labels.append(f'{wait_bins[j]}-{wait_bins[j+1]-1}')
    ...
    # Annotate N
    for j, (b, n) in enumerate(zip(bars, n_vals)):
        ax.annotate(f'n={n}', xy=(b.get_x() + b.get_width() / 2,
                    min(rho_vals[j], 0) - 0.03),
    ...
    ax.set_title('(f)  Cache benefit vanishes\nunder high system load')
...
def panel_g(ax, sel_kv, ttft, sel_waiting, input_tokens):
    # Input length quartiles (same as panel d)
    input_edges, _, short_labels = _quartile_edges_and_labels(input_tokens)
    input_colors = [C_BLUE, C_ORANGE, C_RED, C_PURPLE]

    # Load buckets (same as panel e)
    wait_bins = [0, 1, 2, 3, 5, 8, 15]
    load_labels = []
    valid_load_bins = []
    ...
    for j in range(len(wait_bins) - 1):
        mask = (sel_waiting >= wait_bins[j]) & (sel_waiting < wait_bins[j + 1])
        if mask.sum() > 50:
            load_labels.append('{}-{}'.format(wait_bins[j], wait_bins[j + 1] - 1))
    ...
    for li, (il_label, il_color) in enumerate(zip(short_labels, input_colors)):
        ...
        for k, (b, n) in enumerate(zip(bars, n_vals)):
            y_pos = min(rho_vals[k], 0) - 0.02
            ax.annotate(f'{n}', xy=(b.get_x() + b.get_width() / 2, y_pos),
                        ha='center', va='top', fontsize=5, color=C_GREY)
    ...
    ax.set_xlabel('Waiting requests (system load)')
    ax.set_ylabel('Spearman $\\rho$(KV hit, TTFT)')
    ax.set_title('(h)  Cache effect by load AND request length')
    ax.legend(loc='lower left', framealpha=0.9, fontsize=7, ncol=4)
...
def panel_i(ax, sel_kv, ttft, sel_waiting, input_tokens):
    ...
    ax.set_ylabel('Spearman $\\rho$(KV hit, TTFT)')
    ax.set_title('(i)  Cache effect by request length AND load')
    ax.legend(loc='lower left', framealpha=0.9, fontsize=7, ncol=n_load)
...
def main():
    ...
    # Create figure — 5 rows x 2 cols (9 panels, last row: h + i)
    fig = plt.figure(figsize=(7.2, 16.5))
    gs = gridspec.GridSpec(5, 2, hspace=0.58, wspace=0.38,
                           left=0.09, right=0.95, top=0.96, bottom=0.03)
    ...
    print('  Plotting panel (c): Confound ...')
    panel_b(ax_c, sel_kv, input_tokens)

    print('  Plotting panel (d): Binned nonlinear curve ...')
    panel_c(ax_d, sel_kv, ttft, input_tokens)

    print('  Plotting panel (e): Controlled sign reversal ...')
    panel_d(ax_e, sel_kv, ttft, input_tokens)

    print('  Plotting panel (f): Load modulation ...')
    panel_e(ax_f, sel_kv, ttft, sel_waiting)

    print('  Plotting panel (g): 3-way interaction heatmap ...')
    panel_f(ax_g, sel_kv, ttft, input_tokens, sel_waiting)

    print('  Plotting panel (h): Load × length grouped bars ...')
    panel_g(ax_h, sel_kv, ttft, sel_waiting, input_tokens)

    print('  Plotting panel (i): Length × load grouped bars ...')
    panel_i(ax_i, sel_kv, ttft, sel_waiting, input_tokens)

    fig.suptitle(
        'KV Cache Hit Ratio and TTFT: A Nonlinear, Conditional Relationship\n'
        'Justifying Neural Network Models over Linear Alternatives for LLM Request Routing',
