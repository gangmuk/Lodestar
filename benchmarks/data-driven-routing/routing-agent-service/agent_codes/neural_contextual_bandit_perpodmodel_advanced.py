...
class NeuralContextualBandit:
    ...
    def _create_cluster_features(self, pod_features, kv_hit_ratios):
        """
        Create cluster-wide aggregate features (scale-invariant).
        ...
        # Typical order: [inflight_req, gpu_cache, cpu_cache, running_req, waiting_req, prefill_tok, decode_tok]
        ...
        gpu_cache = pod_features[:, :, 1] if pod_feat_dim > 1 else torch.zeros(batch_size, num_pods, device=device)
        ...
        running_mean = running_reqs.mean(dim=1, keepdim=True)  # [batch, 1]
        running_std = torch.clamp(running_reqs.std(dim=1, keepdim=True), min=min_std)
        z_running = (running_reqs - running_mean) / running_std  # [batch, num_pods]
        ...
        gpu_mean = gpu_cache.mean(dim=1, keepdim=True)
        gpu_std = torch.clamp(gpu_cache.std(dim=1, keepdim=True), min=min_std)
        ...
        rank_by_load = torch.argsort(torch.argsort(total_load, dim=1), dim=1).float() / max(num_pods - 1, 1)
        ...
        cluster_utilization = (running_mean / theoretical_max).expand(-1, num_pods)  # [batch, num_pods]
    ...
    def choose_action(self, pod_features, kv_hit_ratios, request_features, evaluate=False):
        ...
        with torch.no_grad():
            ...
            if evaluate or self.exploration_method == 'greedy':
                ...
                # NEW: Epsilon-greedy with visit-weighted exploration
                if np.random.random() < self.epsilon:
                    # Visit-weighted exploration: prefer less-visited pods
                    exploration_weights = 1.0 / (self.action_counts + 1)
                    exploration_probs = exploration_weights / exploration_weights.sum()
                    action = np.random.choice(self.action_dim, p=exploration_probs)
                    explored = True
                else:
                    action = int(np.argmax(predicted_rewards))
                    explored = False
            ...
            # NEW: Adaptive epsilon adjustment (check every 100 steps)
    ...
    def remember(self, pod_features, kv_hit_ratios, request_features, action, reward, input_tokens=None):
        ...
        # NEW: Track routing history for temporal features
        self.routing_history.append({
            'timestamp': time.time(),
            'pod': action if isinstance(action, int) else action.item(),
