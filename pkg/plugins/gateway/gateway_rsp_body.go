...
func (s *Server) handleStreamingResponse(requestID string, responseBody []byte) (openai.CompletionUsage, bool, *extProcPb.ProcessingResponse) {
	...
	for streaming.Next() {
		...
		if len(evt.Choices) > 0 && evt.Choices[0].Delta.Content != "" {
			if timing.firstTokenTime.IsZero() { // First token response. End of prefill phase, start of decode phase
...
func (s *Server) HandleResponseBody(ctx context.Context, req *extProcPb.ProcessingRequest, user utils.User, rpm int64, model string, stream bool, traceTerm int64, hasCompleted bool) (*extProcPb.ProcessingResponse, bool) {
	...
	if usage.TotalTokens != 0 {
		...
		klog.V(5).Infof("request end, requestID: %s - %s", routerCtx.RequestID, requestEnd)
...
func (s *Server) calculateTimingMetrics(timing *RequestTiming, currentTime time.Time, routerCtx *types.RoutingContext, stream bool, numInputTokens int64, numOutputTokens int64, numTotalTokens int64) ([]*configPb.HeaderValueOption, string) {
	...
	numInflightPrefillRequestsAllPods := utils.GetSnapshotNumInflightPrefillRequestsForRequest(routerCtx.RequestID)
	headers, jsonStrings["numInflightPrefillRequestsAllPods"] = addMetricToHeaders(headers, HeaderNumInflightPrefillRequestsForAllPods, numInflightPrefillRequestsAllPods, utils.GetrequestInflightMutex())

	numInflightDecodeRequestsAllPods := utils.GetSnapshotNumInflightDecodeRequestsForRequest(routerCtx.RequestID)
	...
	headers, jsonStrings["numPrefillTokensForAllPods"] = addMetricToHeaders(headers, HeaderNumInflightPrefillTokensForAllPods, numInflightPrefillTokensForAllPods, &utils.RequestToNumInflightPrefillTokensMutex)

	numInflightDecodeTokensForAllPods := utils.GetSnapshotNumInflightDecodeTokensForRequest(routerCtx.RequestID)
	headers, jsonStrings["numDecodeTokensForAllPods"] = addMetricToHeaders(headers, HeaderNumInflightDecodeTokensForAllPods, numInflightDecodeTokensForAllPods, &utils.RequestToNumInflightDecodeTokensMutex)
	...
	/// sliding window logic based pod metrics
	ts := time.Now()
	podDetailedMetrics := utils.GetRequestPodMetrics(routerCtx.RequestID)
	klog.V(5).Infof("GetRequestPodMetrics took %d, %s, %s", time.Since(ts).Milliseconds(), routerCtx.RequestID, selectedPodIP)
