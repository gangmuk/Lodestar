...
"""
Model Explainability Suite for Latency Predictor
-------------------------------------------------
This script loads a trained latency predictor from a model directory and performs
...
Usage example:
  python latency_predictor_explainability.py \
    --final_model_dir /mnt/data/projects/aibrix-gangmuk/benchmarks/data-driven-routing/routing-agent-service/workload-and-experiment_results/SharingRatio28%/latency_predictor_ttft-trained_on_merged-data_all-iter10-20251011_063005/final_model
"""
...
import latency_predictor as lp
...
def _forward_latencies(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor) -> torch.Tensor:
    """Forward pass to get predicted latencies for each pod"""
    pod = _to_batch(pod).to(_device())
    kv = _to_batch(kv).to(_device())
    req = _to_batch(req).to(_device())
    with torch.no_grad():
        latencies = predictor.network(pod, kv, req)
    return latencies
...
def gradient_saliency(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, topk: int = 10, target_pod: int = 0) -> Dict[str, Any]:
    """Compute gradient-based saliency for latency predictions"""
    pod = pod.clone().detach().requires_grad_(True)
    kv = kv.clone().detach().requires_grad_(True)
    req = req.clone().detach().requires_grad_(True)
    latencies = predictor.network(pod, kv, req)
    
    # Explain the latency prediction for target pod
    target_latency = latencies[:, target_pod]
    target_latency.sum().backward()

    # Focus on target pod's feature channels
...
def integrated_gradients(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, steps: int = 32, target_pod: int = 0) -> Dict[str, Any]:
    """Integrated gradients for latency predictor"""
    # Baseline: zeros
    ...
    for alpha in np.linspace(0.0, 1.0, steps, endpoint=True):
        ...
        latencies = predictor.network(pod_alpha, kv_alpha, req_alpha)
        target_latency = latencies[:, int(target_pod)]
        grads = torch.autograd.grad(target_latency.sum(), [pod_alpha, kv_alpha, req_alpha], allow_unused=True)
    ...
    # Also compute absolute-IG magnitude
...
def permutation_symmetry_test(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, trials: int = 20) -> Dict[str, Any]:
    """Test if predictions are invariant to pod permutation"""
    n = pod.shape[1]
    agree = 0
    mean_l1 = 0.0
    for _ in range(trials):
        ...
        latencies_orig = _forward_latencies(predictor, pod, kv, req)[0].detach().cpu().numpy()
        pod_perm = pod[:, perm, :]
        kv_perm = kv[:, perm, :]
        latencies_perm = _forward_latencies(predictor, pod_perm, kv_perm, req)[0].detach().cpu().numpy()

        # Undo permutation on latencies_perm for comparison
        latencies_perm_unperm = latencies_perm[inv]
        agree += int(np.argmin(latencies_orig) == np.argmin(latencies_perm_unperm))
        mean_l1 += float(np.mean(np.abs(latencies_orig - latencies_perm_unperm)))
    return {
...
def pdp_ice(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, ranges: Dict[str, np.ndarray], num_points: int = 25, num_contexts: int = 16, target_pod: int = 0) -> Dict[str, Any]:
    """Partial Dependence Plots for latency predictions"""
    ...
    for f in range(pod_feat_dim):
        ...
        for c in range(pod_ctx.shape[0]):
            ...
            for v in grid:
                ...
                latencies = _forward_latencies(predictor, pod_tmp, kv_ctx[c:c+1], req_ctx[c:c+1])[0].detach().cpu().numpy()
                latency_curve.append(latencies[target_pod])
            ice_curves.append(latency_curve)
    ...
    for f in range(kv_dim):
        ...
        for c in range(kv_ctx.shape[0]):
            ...
            for v in grid:
                ...
                latencies = _forward_latencies(predictor, pod_ctx[c:c+1], kv_tmp, req_ctx[c:c+1])[0].detach().cpu().numpy()
                latency_curve.append(latencies[target_pod])
            ice_curves.append(latency_curve)
        pdp = np.mean(np.array(ice_curves), axis=0)
    ...
    for f in range(req_dim):
        ...
        for c in range(req_ctx.shape[0]):
            ...
            for v in grid:
                ...
                req_tmp[:, f] = float(v)
                latencies = _forward_latencies(predictor, pod_ctx[c:c+1], kv_ctx[c:c+1], req_tmp)[0].detach().cpu().numpy()
                latency_curve.append(latencies[target_pod])
            ice_curves.append(latency_curve)
        pdp = np.mean(np.array(ice_curves), axis=0)
...
def relative_pdp(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, ranges: Dict[str, np.ndarray], f_idx: int, num_points: int = 25, num_contexts: int = 16, target_pod: int = 0) -> Dict[str, Any]:
    """Relative PDP keeping sum constant across pods"""
    ctx_idx = torch.randperm(pod.shape[0])[: min(num_contexts, pod.shape[0])]
    pod_ctx = pod[ctx_idx].clone()
    kv_ctx = kv[ctx_idx].clone()
    req_ctx = req[ctx_idx].clone()
...
def ale_one_feature(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, f_idx: int, bins: int = 10, target_pod: int = 0) -> Dict[str, Any]:
    ...
    for i in range(bins):
        ...
        lat_a = _forward_latencies(predictor, pod_a, kv[idx], req[idx])[:, target_pod].detach().cpu().numpy()
        lat_b = _forward_latencies(predictor, pod_b, kv[idx], req[idx])[:, target_pod].detach().cpu().numpy()
        effects.append(float(np.mean(lat_b - lat_a)))
        centers.append((a + b) / 2.0)
...
def rank_effect(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, f_idx: int, target_pod: int = 0) -> Dict[str, Any]:
    """Relationship between target pod's rank by feature and its latency"""
    with torch.no_grad():
        latencies = _forward_latencies(predictor, pod, kv, req)[:, target_pod].detach().cpu().numpy()
    ...
    means = []
    for r in uniq:
        means.append(float(np.mean(latencies[ranks == r])))
    return {"ranks": uniq.tolist(), "mean_latency": means}


def counterfactual_min_change(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, ranges: Dict[str, np.ndarray], steps: int = 50, step_size: float = 0.1) -> Dict[str, Any]:
    """Find minimal changes to flip the selected pod"""
    pod0 = pod[:1].clone().detach().to(_device())
    kv0 = kv[:1].clone().detach().to(_device())
    req0 = req[:1].clone().detach().to(_device())

    with torch.no_grad():
        base_latencies = predictor.network(pod0, kv0, req0)[0]
        a = int(torch.argmin(base_latencies).item())  # Best pod (lowest latency)
    results = {}

    num_pods = base_latencies.shape[0]
    for b in range(num_pods):
        ...
        for _ in range(steps):
            ...
            latencies = predictor.network(pod_cf, kv_cf, req_cf)[0]
            # Want to make pod b better (lower latency) than pod a
            margin = latencies[a] - latencies[b]
            ...
            with torch.no_grad():
                ...
                latencies = predictor.network(pod_cf, kv_cf, req_cf)[0]
                if int(torch.argmin(latencies).item()) == b:
                    success = True
                    break
...
def interaction_heatmap(predictor: lp.LatencyPredictor, pod: torch.Tensor, kv: torch.Tensor, req: torch.Tensor, ranges: Dict[str, np.ndarray], target_pod: int = 0, f1: int = 0, f2: int = 1, num_points: int = 30) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """2D interaction heatmap for two pod features"""
    pod0 = pod[:1].clone()
    kv0 = kv[:1].clone()
    req0 = req[:1].clone()
    ...
    for i, xv in enumerate(x_vals):
        for j, yv in enumerate(y_vals):
            ...
            Z[j, i] = _forward_latencies(predictor, pod_tmp, kv0, req0)[0, target_pod].item()
...
def _add_saliency_page(pdf: PdfPages, grad_res: Dict[str, Any], names: Dict[str, list], topk: int = 20):
    ...
    ax.set_title("Gradient Saliency for Latency Prediction (combined: pod, kv, request)")
    ax.set_ylabel("|dLatency/dx|")
    ax.grid(True, alpha=0.3)
    from matplotlib.patches import Patch
    legend_elems = [Patch(facecolor='tab:blue', label='positive (increases latency)'), Patch(facecolor='tab:red', label='negative (decreases latency)')]
...
def _add_ig_page(pdf: PdfPages, ig_res: Dict[str, Any], names: Dict[str, list], topk: int = 20):
    ...
    ax.set_xticks(range(len(values)))
    ax.set_xticklabels(labels, rotation=45, ha='right', fontsize=8)
    ax.set_title("Integrated Gradients for Latency Prediction (combined: pod, kv, request)")
...
def _add_counterfactuals_page(pdf: PdfPages, cf_res: Dict[str, Any]):
    ...
    if entries:
        ...
        ax.set_title("Counterfactual Distance to Flip to Each Pod (green=success, red=failed)")
        ax.grid(True, alpha=0.3)
    else:
...
def _add_pdp_pages(pdf: PdfPages, pdp_res: Dict[str, Any], target_pod: int, names: Dict[str, list], n_cols: int = 4, n_rows: int = 2):
    ...
    for i in range(0, len(combined), per_page):
        ...
        for ax, (group, pretty_name, obj) in zip(axes_flat, chunk):
            ...
            ax.set_ylabel(f"Latency(pod{target_pod})", fontsize=8)
            ax.grid(True, alpha=0.3)
...
def _add_interaction_page(pdf: PdfPages, x_vals: np.ndarray, y_vals: np.ndarray, Z: np.ndarray, title: str):
    ...
    plt.colorbar(im, ax=ax, label='Predicted Latency')
...
def _add_ale_page(pdf: PdfPages, ale_obj: Dict[str, Any], title: str):
    ...
    ax.set_ylabel("Accumulated local effect on latency")
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    pdf.savefig(fig, bbox_inches='tight')
    plt.close(fig)
...
def _add_findings_pages(pdf: PdfPages, grad_res: Dict[str, Any], ig_res: Dict[str, Any], sym_res: Dict[str, Any], cf_res: Dict[str, Any], pdp_res: Dict[str, Any], names: Dict[str, list]):
    ...
    plt.axis('off')
    text = [
        "EXPLAINABILITY FINDINGS - LATENCY PREDICTOR",
        "",
        "Saliency (local sensitivity to latency):",
    ...
    fig = plt.figure(figsize=(11, 8.5))
    plt.axis('off')
    lines = ["PDP Trend Summary (latency trends)", "", "Pod features:"] + pod_lines + ["", "KV features:"] + kv_lines + ["", "Request features:"] + req_lines
    plt.text(0.05, 0.95, "\n".join(lines), va='top', ha='left', fontsize=11)
    pdf.savefig(fig, bbox_inches='tight')
    plt.close(fig)

    # Symmetry + Counterfactuals
    entries = []
    for k, v in cf_res.items():
        if k.startswith("to_pod_"):
            try:
...
def main():
    parser = argparse.ArgumentParser(description="Explainability for Latency Predictor")
    parser.add_argument("--final_model_dir", required=True, help="Path to trained model directory containing latency_predictor.pth")
    ...
    grad_res = gradient_saliency(predictor, pod_ctx, kv_ctx, req_ctx, topk=10)
    ...
    ig_res = integrated_gradients(predictor, pod_ctx, kv_ctx, req_ctx, steps=32)
    with open(os.path.join(out_dir, "integrated_gradients.json"), "w") as f:
        json.dump(ig_res, f, indent=2)

    logger.info("Running symmetry test...")
    sym_res = permutation_symmetry_test(predictor, pod_ctx[:1], kv_ctx[:1], req_ctx[:1], trials=30)
    with open(os.path.join(out_dir, "symmetry_test.json"), "w") as f:
        json.dump(sym_res, f, indent=2)

    logger.info("Computing PDP/ICE...")
    target_pod = 0
    pdp_res = pdp_ice(predictor, pod_ctx, kv_ctx, req_ctx, ranges, num_points=25, num_contexts=min(16, pod_ctx.shape[0]), target_pod=target_pod)
    with open(os.path.join(out_dir, "pdp_ice.json"), "w") as f:
        json.dump(pdp_res, f, indent=2)

    logger.info("Computing counterfactuals...")
    cf_res = counterfactual_min_change(predictor, pod_ctx, kv_ctx, req_ctx, ranges, steps=60, step_size=0.15)
    ...
    if len(top_pod_feats) >= 2:
        ...
        logger.info(f"Computing interaction heatmap for features {f1} and {f2}...")
        x_vals, y_vals, Z = interaction_heatmap(predictor, pod_ctx, kv_ctx, req_ctx, ranges, target_pod=target_pod, f1=f1, f2=f2, num_points=40)
        interaction_payload = (x_vals, y_vals, Z, f1, f2)

    # Advanced analyses for prefill_tokens if present
    ...
    if prefill_idx is not None:
        logger.info(f"Computing advanced analyses for {prefill_name}...")
        rel_pdp_obj = relative_pdp(predictor, pod_ctx, kv_ctx, req_ctx, ranges, f_idx=int(prefill_idx), num_points=25, num_contexts=min(16, pod_ctx.shape[0]), target_pod=target_pod)
        ale_obj = ale_one_feature(predictor, pod_ctx, kv_ctx, req_ctx, f_idx=int(prefill_idx), bins=10, target_pod=target_pod)
        rank_obj = rank_effect(predictor, pod_ctx, kv_ctx, req_ctx, f_idx=int(prefill_idx), target_pod=target_pod)

    # Generate PDF report
    logger.info("Generating PDF report...")
    report_pdf = os.path.join(out_dir, "xai_report.pdf")
    with PdfPages(report_pdf) as pdf:
        _add_title_page(pdf, "Latency Predictor Explainability Report", os.path.basename(final_model_dir))
