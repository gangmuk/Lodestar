...
class AzureWorkloadGenerator:
    ...
    def redistribute_timestamps_by_access_pattern(self, timestamps, prefix_groups, duration_seconds, access_pattern='sequential', normal_mean_ratio=0.5, normal_std_ratio=0.2):
        ...
        if access_pattern == 'sequential':
            # Cluster each group's requests contiguously in time, in group_id
            # order. Group 0's members get the earliest timestamps, then group 1,
            # etc. Within a group, original index order is preserved.
            #
            # This is what produces real temporal locality: the first request of
            # a group is the cold prefix-cache miss; the remaining group members
            # arrive back-to-back and hit the warm prefix. The exact RPS pattern
            # is preserved because the timestamp *set* is unchanged — only the
            # request → timestamp assignment is permuted.
            order = sorted(range(num_requests),
                           key=lambda i: (prefix_groups[i][0], i))
            new_timestamps = [0] * num_requests
            for slot, request_idx in enumerate(order):
                new_timestamps[request_idx] = sorted_timestamps[slot]
    ...
    def generate_workload(self, config):
        ...
        tokens_per_hash_id = config.get('tokens_per_hash_id', 128)
        ...
        # Redistribute timestamps based on access pattern. Always runs — even
        # 'sequential' now genuinely clusters group members in time (the old
        # behaviour was a no-op that scattered group members at random).
        ...
        workload_data = []
        prefix_contents = {}  # group_id -> shared text
        group_shared_hash_ids = {}  # group_id -> list of shared hash_ids
        next_hash_id = 1  # 0 reserved as a 'root' sentinel
        ...
        for i in tqdm(range(num_total_requests), desc="Generating prompts", unit="req"):
            ...
            # On first encounter of a group: build shared text + allocate shared
            # hash_ids that every member of this group will reuse. Subsequent
            # group members get exactly the same shared hash_ids, so a temporal
            # trie over hash_ids correctly counts the shared prefix as a hit
            # only for the requests that arrive after the group's first one.
            if group_id not in prefix_contents:
                ...
                n_shared_blocks = math.ceil(shared_length / tokens_per_hash_id) if shared_length > 0 else 0
                group_shared_hash_ids[group_id] = list(range(next_hash_id, next_hash_id + n_shared_blocks))
                next_hash_id += n_shared_blocks

            shared_content = prefix_contents[group_id]
            shared_hids = group_shared_hash_ids[group_id]

            # Generate unique suffix (fresh per-request)
            unique_content = generate_realistic_prompt(self.tokenizer, unique_length)
            unique_content = adjust_prompt_to_length(self.tokenizer, unique_content, unique_length)

            n_unique_blocks = math.ceil(unique_length / tokens_per_hash_id) if unique_length > 0 else 0
            unique_hids = list(range(next_hash_id, next_hash_id + n_unique_blocks))
            next_hash_id += n_unique_blocks
            ...
            workload_data.append({
                ...
                "hash_ids": shared_hids + unique_hids,
            })
...
def main():
    ...
    parser.add_argument("--tokens-per-hash-id", type=int, default=128,
                        help="Tokens per hash_id block for the prefix-trie metric (default 128).")
    ...
    config = {
        ...
        "tokens_per_hash_id": args.tokens_per_hash_id,
        "seed": args.seed
    }
    ...
    # Always run plot_data_distribution.py (lives in workloads/mooncake) so the
    # temporally-correct, trie-based prefix-hit-ratio plot is produced alongside
    # the workload. hash_ids in the JSONL drive the trie.
    workload_jsonl = os.path.join(output_dir, "workload.jsonl")
    distribution_pdf = os.path.join(output_dir, "workload_distribution.pdf")
    plot_script = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "mooncake",
